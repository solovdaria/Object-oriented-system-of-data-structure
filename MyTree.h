...
This class works with tree and it's functions. Such as input values,
...
private:
	/*!
	\brief Subclass of class MyTree

	This subclass is responsible for the tree node, it's descents, and values in the node
	...
	*/
	...
	node* tree; ///< Tree node
public:
	node* GetTree() ///< Tree getter
	{
		return tree;
	}

	MyTree() ///< Constructor by default
	{
		tree = nullptr;
	}
	...
	/*!
	\brief Function for the tree input
	...
	*/
	int SearchRecursion(node* curr, type1 v, int counter = 1) 
	{
	...
	\return Level of the searched value in case it was in the tree
	*/
	int SearchLoop(node* curr, type1 v, int counter = 1) 
	{
		while (curr != nullptr)
		{
			int tmp = curr->info * 1000;
